      ...
      return new Promise((resolve, reject) => cognitoUser.authenticateUser(authDetails, {
        ...
        onSuccess: (session, userConfirmationNecessary) => {
          ...
          const mutationPayload = {
            ...
            username: payload.username,
            idTokenJwt: session.getIdToken().getJwtToken(),
            idTokenExpiration: session.getIdToken().getExpiration(),
            refreshToken: session.getRefreshToken().getToken(),
            accessTokenJwt: session.getAccessToken().getJwtToken(),
            accessTokenExpiration: session.getAccessToken().getExpiration(),
          };
      ...
      return new Promise((resolve, reject) => {
        cognitoUserPool.signUp(
          ...
          (err, data) => {
            ...
            commit(types.SIGNUP_FAILURE, { errorMessage: err.message });
            reject();
          });
      });
    },
    ...
    confirmRegistration({ commit }, payload) {
      ...
      return new Promise((resolve, reject) => {
        cognitoUser.confirmRegistration(payload.code, true, (err) => {
          if (!err) {
            commit(types.CONFIRMED);
            resolve();
            return;
          }
          commit(types.CONFIRMED_FAILURE, { errorMessage: err.message });
          reject();
        });
      });
    },
